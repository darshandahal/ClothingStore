...
export default function Sales() {
  ...
  const getDailySalesChart = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayData = { day: dayName, sales: 0, orders: 0 };
  ...
  const getMonthlySalesChart = () => {
    ...
    return monthNames.map((month, index) => {
      const monthData = { month, sales: 0, orders: 0 };

      salesData.forEach((sale) => {
        const saleDate = new Date(sale.created_at);
        if (saleDate.getMonth() === index && saleDate.getFullYear() === currentYear) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Track your daily and monthly sales performance</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-gradient-to-br from-purple-500 to-purple-600 text-white rounded-lg shadow-lg p-6">
                ...
                <p className="text-xs opacity-80 mt-1">{new Date().toLocaleDateString("en-US", { month: "long" })}</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-lg shadow-lg p-6">
                <h2 className="text-lg font-semibold text-gray-800 mb-4">Last 7 Days Sales (Rs.)</h2>
              ...
              <div className="bg-white rounded-lg shadow-lg p-6">
                <h2 className="text-lg font-semibold text-gray-800 mb-4">Monthly Sales Trend (Rs.)</h2>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {salesData.slice(0, 10).map((sale) => (
                      <tr key={sale.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm text-gray-900">{new Date(sale.created_at).toLocaleString()}</td>
                        <td className="px-4 py-3 text-sm text-gray-900 font-mono">#{sale.id.slice(0, 8)}</td>
                        <td className="px-4 py-3 text-sm text-gray-900">{sale.customer_name || "Guest"}</td>
                        <td className="px-4 py-3 text-sm text-gray-900">{sale.total_items || 0} items</td>
                ...
                {salesData.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-gray-400 mt-2">Sales will appear here once customers make purchases.</p>
                  </div>
                )}
              </div>
            </div>
